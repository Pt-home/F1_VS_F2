...
self.onmessage = async (ev) => {
  ...
  try {
    ...
    if (mode === "RANDOM") {
      const modes = [
        "F1_VS_F2","F2_VS_F1","F1_VS_INDEX","F2_VS_INDEX","INDEX_VS_F1","INDEX_VS_F2",
...
function buildXY(space, rng) {
  ...
  if ((space.mode || "grid") === "grid") {
    ...
    // choose EXACTLY ONE step (no mixing), with fallback and clamp
...
// deterministic stepping; inclusive bounds with guard; no float accumulation
function rangeStepped(a, b, step) {
  if (!(step > 0)) return [a];
  const span = b - a;
  const cnt = Math.floor(span / step + 1e-9) + 1;
